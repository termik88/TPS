...
@PipelineDecorator.component(return_values=['path, labels, subs'], execution_queue="default")
def step_one(path):
   import os
   import pandas as pd
...
def step_five(y_val, y_pred_proba_val):
   ...
   # Расчет log_loss
   logloss = log_loss(y_val, y_pred_proba_val)
...
def executing_pipeline(parameters):
   ...
   task = Task.init(project_name="TPS", task_name="TPS_pipeline_v6")
   ...
   lib()
   path, labels, subs, sub_ids = step_one(path)
